...
chl_0_0 <- raster::projectRaster(chl_0_0, crs = CRS4326,over=T,progress='text')
...
#         labels=4, lines=0)
# Plot the lon-lat dist of our clustering
result_8b <- cbind(group_8$cluster,cadeau_00_st)
result_8b$group <- as.factor(group_8$cluster) 
#mapview(result_8b, xcol = "x", ycol = "y", zcol="group",crs = 4269, grid = FALSE, cex=0.5)



result_cadeau_8trial_sf <- st_as_sf(result_8b, coords = c("x", "y"), crs = my.projection)
st_crs(result_cadeau_8trial_sf)
library(stars)
result_cadeau_8trial_sf<-st_rasterize(result_cadeau_8trial_sf  %>% dplyr::select(group, geometry))
stars::write_stars(result_cadeau_8trial_sf, file.path(dir_cluster, "result_cadeau_8trial_sf_2.tif"))
