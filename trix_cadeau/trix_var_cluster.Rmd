...
pr <- result_4b %>% select(c(group,x,y))
# Define coordinate reference system

# Create sf object
pr_lat_long_sf <- st_as_sf(pr, coords = c("x", "y"), crs = 4326)
st_crs(pr_lat_long_sf)
...
ncovr_queen <- st_rook(pr_lat_long_sf)

#pr_lat_long_sf_4326 <- st_transform(pr_lat_long_sf,4326)
#str_c(pr_lat_long_sf_4326)
# Export shapefile
st_write(pr_lat_long_sf, "G:/Il mio Drive/ASSEGNO DI RICERCA/SHAREMED_mio/wp4_atlas/TRIX/pr_lat_long_sf", driver="ESRI Shapefile")

"G:\Il mio Drive\ASSEGNO DI RICERCA\SHAREMED_mio\wp4_atlas\TRIX\pr_lat_long_sf\pr_lat_long_sf_q.shp"

pr <- st_read("G:/Il mio Drive/ASSEGNO DI RICERCA/SHAREMED_mio/wp4_atlas/TRIX/pr_lat_long_sf/pr_lat_long_sf.shp")

pr_lat_long_sppnt <- SpatialPointsDataFrame(pr_lat_long_sf)

library(tidyverse)
library(sf)
library(stars)
pr_torast<-st_rasterize(pr_lat_long_sf %>% dplyr::select(group, geometry))
write_stars(pr_torast, "G:/Il mio Drive/ASSEGNO DI RICERCA/SHAREMED_mio/wp4_atlas/TRIX/pr_torast.tif")
here()
```
...
xy <- cadeau_00_st %>% select(x,y)
dmat <-cadeau_00_st %>%select(4:9)

library (spatialcluster)
scl <- scl_full (xy, dmat, ncl = 2)
plot (scl)

clusters <- redcap(4, queen_w, cadeau_00_st, "fullorder-completelinkage")
...
clusters_cadeau_save<- readRDS("clusters_cadeau.RData")
...
lag_din <- spatial_lag(queen_w, cadeau_00_st_lat_long_sf ['din'])
lag_din
```
