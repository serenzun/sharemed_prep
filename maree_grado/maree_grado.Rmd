...
maree_abs<- merged_df %>% 
  ...
  select(c(1,4:6)) %>% rename(Year=FileName)
  
plot(maree_abs$Year,maree_abs$Thr_140)

melted_df <- maree_abs %>%
  tidyr::pivot_longer(cols = c(2:4), names_to = "Variable", values_to = "Height")

library(ggplot2)

ggplot(melted_df, aes(x = Year, y = Height, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge")+
  labs(title = "Bar Plot of Values by Year", x = "Year", y = "Value") +
